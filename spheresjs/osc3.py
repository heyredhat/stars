...
momentum = vp.arrow(color=vp.color.red)
...
while True:
    ...
    #print("mo:")
    #print(qt.expect(p, state))
    #print("*")
    #state = (-2 * np.pi * 1j * H * dt).expm()*state
    #sphere.state = state
    sphere.center = vp.vector(qt.expect(x, sphere.state),0,0)
    momentum.pos = sphere.center
    momentum.axis = vp.vector(qt.expect(p, sphere.state),0,0)
    sphere.radius = qt.expect(n, sphere.state)
    #sphere.radius = qt.expect(p, state)**2
    #sphere.color = vp.color.hsv_to_rgb(vp.vector(qt.expect(p, state)**2,1,1))
