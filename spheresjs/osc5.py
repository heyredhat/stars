...
class MajoranaSphere:
    ...
    def keyboard(self, event):
        ...
        if key == "a":
            ...
            xX = qt.tensor((aX + aX.dag())/np.sqrt(2), qt.identity(modes), qt.identity(modes))
            pX = qt.tensor(-1j * (aX - aX.dag())/np.sqrt(2), qt.identity(modes), qt.identity(modes))
            ...
            xY = qt.tensor(qt.identity(modes), (aY + aY.dag())/np.sqrt(2), qt.identity(modes))
            pY = qt.tensor(qt.identity(modes), -1j * (aY - aY.dag())/np.sqrt(2), qt.identity(modes))
            ...
            xZ = qt.tensor(qt.identity(modes), qt.identity(modes), (aZ + aZ.dag())/np.sqrt(2))
            pZ = qt.tensor(qt.identity(modes), qt.identity(modes), -1j * (aZ - aZ.dag())/np.sqrt(2))

            H = 2 * np.pi * freq * ((aX.dag() * aX + 0.5)+\
                                    (aY.dag() * aY + 0.5)+\
                                    (aZ.dag() * aZ + 0.5))
            self.energy = H
        elif key == "[":
...
Sx = qt.tensor(qt.identity(modes**3), qt.sigmax())
Sy = qt.tensor(qt.identity(modes**3), qt.sigmay())
Sz = qt.tensor(qt.identity(modes**3), qt.sigmaz())

Jx = qt.tensor(Lx, qt.identity(spin_d)) + qt.tensor(qt.identity(modes**3), qt.sigmax())
Jy = qt.tensor(Ly, qt.identity(spin_d)) + qt.tensor(qt.identity(modes**3), qt.sigmay())
Jz = qt.tensor(Lz, qt.identity(spin_d)) + qt.tensor(qt.identity(modes**3), qt.sigmaz())
...
H = 2 * np.pi * freq * ((aX.dag() * aX + 0.5)+\
                        (aY.dag() * aY + 0.5)+\
                        (aZ.dag() * aZ + 0.5)) + x_term + y_term + z_term

state = qt.tensor(qt.rand_ket(modes), qt.rand_ket(modes), qt.rand_ket(modes))
...
#Xmomentum = vp.arrow(color=vp.color.red, opacity=0.5)
#Ymomentum = vp.arrow(color=vp.color.red, opacity=0.5)
#Zmomentum = vp.arrow(color=vp.color.red, opacity=0.5)

momentum = vp.arrow(color=vp.color.red, opacity=0.5)
#Xangular = vp.arrow(color=vp.color.yellow)
#Yangular = vp.arrow(color=vp.color.yellow)
#Zangular = vp.arrow(color=vp.color.yellow)

angular = vp.arrow(color=vp.color.yellow)
...
while True:
    ...
    copy.dims = [[modes, modes, modes], [1, 1, 1]]
    sphere.center = vp.vector(qt.expect(xX, copy),\
                              qt.expect(xY, copy),\
                              qt.expect(xZ, copy))
    #Xmomentum.pos = sphere.center
    ##Ymomentum.pos = sphere.center
    #Zmomentum.pos = sphere.center
    #Xmomentum.axis = vp.vector(qt.expect(pX, copy), 0, 0)
    #Ymomentum.axis = vp.vector(0, qt.expect(pY, copy), 0)
    #Zmomentum.axis = vp.vector(0, 0, qt.expect(pZ, copy))
    momentum.pos = sphere.center
    momentum.axis = vp.vector(qt.expect(pX, copy), qt.expect(pY, copy), qt.expect(pZ, copy))

    #Xangular.pos = sphere.center
   # Yangular.pos = sphere.center
    #Zangular.pos = sphere.center
   ##Xangular.axis = vp.vector(10*qt.expect(Lx, copy), 0, 0)
    #Yangular.axis = vp.vector(0, 10*qt.expect(Ly, copy), 0)
    #Zangular.axis = vp.vector(0, 0, 10*qt.expect(Lz, copy))
    angular.pos = sphere.center
    angular.axis = vp.vector(qt.expect(Lx, copy), qt.expect(Ly, copy), qt.expect(Lz, copy))

    arrow.axis = sphere.center
   # sphere.radius = qt.expect(n, sphere.state)
    #vp.scene.camera.forward = angular.axis
